...
let rec vars t = match t with
  | Variable v -> [v]
  | List_using_bar (t1, t2) -> vars(t1) @ vars(t2)
  | Anonymous_variable -> []
  | Constant s -> []
...
let rec find_list_subs element ls = match ls with 
  | [] -> printf "%s" element; Variable element
  | (var, tree) :: tail -> 

    if var = element then (printf "found for %s" var; tree) else find_list_subs element tail
;;

let rec substitute given_substitutions t = match t with
  | Variable x -> find_list_subs x given_substitutions
  | List_using_bar(t1, Variable v) ->
    let x = (find_list_subs v given_substitutions) in (
      match x with 
      | List l -> List (t1 :: l)
      | Variable x -> t
      | _ -> raise Error
    )
  | List_using_bar(_, _) -> t
...
let rec mgu_term (term1 : Types.term) (term2 : Types.term) initial_sub = print_substitution_comp initial_sub; printf "\ndone\n"; match term1 with
  | Variable v1 -> (
      match term2 with
      | Variable v2  -> if v1 = v2 then [] else [ (v1, Variable v2)]
      | List_using_bar (hd, bd) -> if (List.mem v1 (vars term2)) then raise MGU_not_exist else [(v1, term2)]
  ...
  | List_using_bar (hd, Variable bd) -> (
    match term2 with
      | Variable v2  -> if (List.mem v2 (vars term1)) then raise MGU_not_exist else [(v2, term1)]
      | List_using_bar (hd2, Variable bd2) -> 
        let curr_sub = mgu_term hd hd2 initial_sub in (bd, Variable bd2) :: curr_sub
      | List_using_bar (hd2, _) -> raise MGU_not_exist
      | Anonymous_variable -> initial_sub
      ...
      | List tl1 -> (
        ...
        | hd1 :: bd1 -> (
          try
            let curr_mgu = (mgu_term (substitute initial_sub hd1) (substitute initial_sub hd) initial_sub) in
            let new_subs = comp_subs initial_sub curr_mgu in
            (bd, List bd1) :: new_subs
          with
  ...
  | List_using_bar (hd, _) -> raise MGU_not_exist
  ...
  | Function (p1, t1) -> (
      match term2 with
      | Variable v2  -> if (List.mem v2 (vars term1)) then raise MGU_not_exist else [(v2, term1)]
      | List_using_bar (hd, bd) -> raise MGU_not_exist
      | Anonymous_variable -> initial_sub
      | Constant c -> raise MGU_not_exist
      | Function (p2, t2) -> (
  ...
  | List tl1 -> (
      ...
      | List_using_bar (hd2, Variable bd2) -> (
        ...
        | hd1 :: bd1 -> (
          try
            let curr_mgu = (mgu_term (substitute initial_sub hd1) (substitute initial_sub hd2) initial_sub) in
            let new_subs = comp_subs initial_sub curr_mgu in
            (bd2, List bd1) :: new_subs
          with
            MGU_not_exist -> raise MGU_not_exist
        )
      )
      | List_using_bar (hd2, _) -> printf ":97979"; raise MGU_not_exist
      | Anonymous_variable -> initial_sub
      | Constant c -> raise MGU_not_exist
      | Function (p, tl) -> raise MGU_not_exist
      | List tl2 -> (printf "099090"; 
  ...
  | Tuple tl1 -> (
      match term2 with
      | Variable v2  -> raise MGU_not_exist
      | List_using_bar (_, _) -> raise MGU_not_exist
      | Anonymous_variable -> initial_sub
      | Constant c -> raise MGU_not_exist
      | Function (p, tl) -> raise MGU_not_exist
      | List tl -> raise MGU_not_exist
  ...
  | ( Function(p1, t1), Function(p2, t2) ) ->(
      if ( (p1 = p2) && ((List.length t1) = (List.length t2))) then (
        let rec mgu_atomic_helper c1 c2 curr_sub = match (c1, c2) with
          ...
          | (head1 :: tail1, head2 :: tail2) ->(
              try
                printf "tern mgu starts\n";
                print_substitution_comp curr_sub;
                let curr_mgu = (mgu_term (substitute curr_sub head1) (substitute curr_sub head2) curr_sub) in
                printf "22222\n";
                let new_subs = comp_subs curr_sub curr_mgu in
                print_substitution_comp new_subs;
